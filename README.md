...
- **听歌**：歌单列表、播放/暂停、上一首/下一首（超 3 秒先回开头）、进度拖动、音量调节、顺序/单曲/随机三种播放模式
- **全屏播放页**：点击播放条封面展开——氛围渐变背景、旋转黑胶唱片、大进度条（Esc 或下滑箭头关闭）
- **演唱会面板**：B 站官方嵌入播放器观看演唱会视频（周杰伦嘉年华II 杭州/南宁/温州/北京鸟巢等场次），按场次分组，记住上次观看
- **体验细节**：明暗双主题（localStorage 记忆）、音量与播放模式持久化、键盘快捷键、Media Session 锁屏媒体控制、PWA 主屏安装
- **响应式**：桌面与手机浏览器均可用
...
```
src/
├── components/       # TrackList / PlayerBar / NowPlaying / ConcertView / ErrorWatcher
├── composables/      # useTheme（明暗主题）、useProgressDrag（进度条拖动）
├── services/
│   ├── source.ts     # MusicSource 音源适配器接口（当前为临时演示源）
│   └── concerts.ts   # 演唱会视频条目（B 站 BV 号维护处）
├── stores/player.ts  # 播放器核心：单例 Audio 引擎 + 队列/进度/模式状态 + Media Session
├── types/track.ts    # Track 数据结构
└── utils/format.ts   # 时间格式化、封面渐变、播放模式标签
```
